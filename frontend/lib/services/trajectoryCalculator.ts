...
class TrajectoryCalculator {
  ...
  calculateTrajectory(nasaObject: NASAObject): TrajectoryData {
    ...
    const points = this.generateTrajectoryPoints(approachAngle, missDistance);
  ...
  private generateTrajectoryPoints(approachAngle: number, missDistance: number): THREE.Vector3[] {
  ...
  calculateImpactConsequences(trajectoryData: TrajectoryData): ImpactConsequences {
  ...
  convertToImpactScenario(nasaObject: NASAObject, targetCity: { lat: number; lng: number; name: string }): ImpactScenario {
    const trajectoryData = this.calculateTrajectory(nasaObject);
    const consequences = this.calculateImpactConsequences(trajectoryData);
    ...
    return {
      ...
      asteroidSize: (nasaObject.estimated_diameter.meters.estimated_diameter_min +
        nasaObject.estimated_diameter.meters.estimated_diameter_max) / 2,
